...
// my
#if 1
class PEM_READ : public Operation {
	public:
		const component::DigestType digestType;
		const uint64_t signatureSize;
		// add
		const uint64_t dataSize;
		const component::Cleartext data1;
		const component::Cleartext data2;
		
		PEM_READ(Datasource& ds, component::Modifier modifier) :
			Operation(std::move(modifier)),
			digestType(ds),
			signatureSize(ds.Get<uint64_t>() % (10 * 1024 * 1024)),
			// add
			dataSize(ds.Get<uint64_t>() % (10 * 1024 * 1024)),
			data1(ds),
			data2(ds)
		{}
		...
		inline bool operator==(const PEM_READ& rhs) const {
			(void)rhs;
			/* TODO */
			return false;
		}
...
// NEW_OP
class NEW_OP : public Operation {
	public:
		const uint64_t dataSize;
		const component::Cleartext data1;
		const component::Cleartext data2;

		NEW_OP(Datasource& ds, component::Modifier modifier) :
			Operation(std::move(modifier)),
			dataSize(ds.Get<uint64_t>() % (10 * 1024 * 1024)),
			data1(ds),
			data2(ds)
		...
		std::string ToString(void) const override;
		nlohmann::json ToJSON(void) const override;
		inline bool operator==(const NEW_OP& rhs) const {
			(void)rhs;
			/* TODO */
			return false;
		}
