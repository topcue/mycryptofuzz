...
// my
#if 1
/* Specialization for operation::PEM_READ */
template<> void ExecutorBase<component::Signature, operation::PEM_READ>::updateExtraCounters(const uint64_t moduleID, operation::PEM_READ& op) const {
	using fuzzing::datasource::ID;
	updateExtraCounters(moduleID, operationID, op.digestType.Get(), op.data1.Get().size(), op.data2.Get().size());
}

template<> void ExecutorBase<component::Signature, operation::PEM_READ>::postprocess(std::shared_ptr<Module> module, operation::PEM_READ& op, const ExecutorBase<component::Signature, operation::PEM_READ>::ResultPair& result) const {
	(void)module;
	(void)op;
}

template<> std::optional<component::Signature> ExecutorBase<component::Signature, operation::PEM_READ>::callModule(std::shared_ptr<Module> module, operation::PEM_READ& op) const {
	return module->OpPEM_READ(op);
}

/* Specialization for operation::NEW_OP */
template<> void ExecutorBase<component::Signature, operation::NEW_OP>::updateExtraCounters(const uint64_t moduleID, operation::NEW_OP& op) const {
	using fuzzing::datasource::ID;
	updateExtraCounters(moduleID, operationID, op.digestType.Get(), op.data1.Get().size(), op.data2.Get().size());
}

template<> void ExecutorBase<component::Signature, operation::NEW_OP>::postprocess(std::shared_ptr<Module> module, operation::NEW_OP& op, const ExecutorBase<component::Signature, operation::NEW_OP>::ResultPair& result) const {
	(void)module;
	(void)op;
}

template<> std::optional<component::Signature> ExecutorBase<component::Signature, operation::NEW_OP>::callModule(std::shared_ptr<Module> module, operation::NEW_OP& op) const {
	return module->OpNEW_OP(op);
}
#endif
...
// my
#if 1
template class ExecutorBase<component::Signature, operation::PEM_READ>;
template class ExecutorBase<component::Signature, operation::NEW_OP>;
#endif
