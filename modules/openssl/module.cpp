...
namespace cryptofuzz {
	namespace module {
		...
		// my
#if 1
		std::optional<component::Signature> OpenSSL::OpPEM_READ(operation::PEM_READ& op) {
			...
			/* Initialize */
			{
				...
				CF_CHECK_EQ(static_cast<size_t>(BIO_write(bio, op.data1.GetPtr(), op.data1.GetSize())), op.data1.GetSize());

				CF_CHECK_NE(bio2 = BIO_new(BIO_s_mem()), nullptr);
				CF_CHECK_EQ(static_cast<size_t>(BIO_write(bio2, op.data2.GetPtr(), op.data2.GetSize())), op.data2.GetSize());
			}
			...
			/* Process */
			{
				...
				try {
					read_bio = ds.Get<bool>();
				} catch ( fuzzing::datasource::Datasource::OutOfData ) {
				}

				try {
					read_OCSP = ds.Get<bool>();
				} catch ( fuzzing::datasource::Datasource::OutOfData ) {
				}

				if(read_bio){
					CF_CHECK_NE(PEM_read_bio(bio, &name, &header, &str, &size), 0);
				}
				/*
				else if(read_OCSP){
					bool read_OCSP_request = true;

					try {
						read_OCSP_request = ds.Get<bool>();
					} catch ( fuzzing::datasource::Datasource::OutOfData ) {
					}

					if(read_OCSP_request)
						PEM_read_bio_OCSP_REQUEST(bio2, out, NULL);
					else
						PEM_read_bio_OCSP_RESPONSE(bio2, out, NULL);
				}
				*/
				else{
					CF_CHECK_NE(inf = PEM_X509_INFO_read_bio(bio2, NULL, NULL, NULL), nullptr);

					CF_CHECK_NE(x = sk_X509_INFO_value(inf, 0), nullptr);

					CF_CHECK_NE(md = toEVPMD(op.digestType), nullptr);

					if (x->x509) {
						X509_get_subject_name(x->x509);
						X509_digest(x->x509, md, digest, NULL);
					} else if (x->crl) {
						X509_CRL_get_issuer(x->crl);
						X509_CRL_digest(x->crl, md, digest, NULL);
					}
				}
			}
		...
		// OpNEW_OP()
		std::optional<component::Signature> OpenSSL::OpNEW_OP(operation::NEW_OP& op) {
			std::optional<component::Signature> ret = std::nullopt;
			std::cout << "[*] OpenSSL::OpNEW_OP()" << std::endl;
